...
export default function GridWidgetConfig({ config, onChange, token }) {
  ...
  const addItemSlot = () => {
    ...
    handleUpdate('items', [...items, { id: Date.now().toString(), widgetId: '' }]);
  };
  ...
  const updateItemWidget = (id, widgetId) => {
    const items = config.items || [];
    handleUpdate('items', items.map(item => item.id === id ? { ...item, widgetId } : item));
  };
  ...
  return (
    <div className="config-group">
      ...
      <div className="config-field" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', paddingBottom: '1rem' }}>
