  ...
  useEffect(() => {
    ...
    fetch(`https://api.github.com/users/${username}`)
      ...
      .catch((err) => {
        ...
        console.warn('GitHub API fetch failed, using fallback mock data:', err.message);
  ...
  return (
    <>
      ...
      <div style={containerStyle} className="github-widget-container">
        {!username ? (
          <div style={{ opacity: 0.7, textAlign: 'center', fontSize: '0.9rem' }}>
            <div style={{ fontSize: '2.5rem', marginBottom: '0.5rem' }}>🐙</div>
            Configure a GitHub username in Settings
          </div>
        ) : loading ? (
          <div className="github-loading" style={{ opacity: 0.8, fontSize: '0.9rem' }}>
          ...
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '0.8rem', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', width: '100%', maxWidth: '280px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', textAlign: 'left' }}>
                <span style={{ fontSize: '1rem', fontWeight: '700', whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }}>
                  {data.name}
                </span>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <span style={{ fontSize: '0.9rem', fontWeight: '800' }}>{data.public_repos}</span>
                <span style={{ fontSize: '0.6rem', opacity: 0.7, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Repos</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <span style={{ fontSize: '0.9rem', fontWeight: '800' }}>{data.followers}</span>
                <span style={{ fontSize: '0.6rem', opacity: 0.7, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Followers</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <span style={{ fontSize: '0.9rem', fontWeight: '800' }}>{data.following}</span>
                <span style={{ fontSize: '0.6rem', opacity: 0.7, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Following</span>
              </div>
            </div>
            ...
            {showGraph && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem', alignItems: 'center' }}>
                <span style={{ fontSize: '0.6rem', opacity: 0.7, alignSelf: 'flex-start', marginLeft: '0.2rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  Contributions
                </span>
                <div style={{
                  display: 'grid',
